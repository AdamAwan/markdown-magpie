...
// across refreshes (the UI relies on it for React keys and edit tracking). The
// flow is part of the key so the same summaries under two flows get distinct ids.
export function clusterId(summaries: string[], flowId?: string): string {
  const hash = createHash("sha1")
    .update(`${flowId ?? ""}\0${[...summaries].sort().join(" ")}`)
    .digest("hex");
...
export function singletonCluster(candidate: GapCandidate): SuggestedGapCluster {
  return buildCluster([candidate], titleFromGapSummary(candidate.summary), undefined, candidate.flowId);
}
