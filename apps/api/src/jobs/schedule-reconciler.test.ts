...
test("reconcile builds crunch + per-task schedules for the default flow", async () => {
  const ctx = makeTestContext();
  await ctx.stores.crunchRuns.updateSettings(undefined, { enabled: true, cron: "0 2 * * *" });
  ...
  assert.deepEqual(await scheduledKeys(ctx), [
    { type: "trigger_scheduled_crunch", key: "flow:default", cron: "0 2 * * *" },
...
test("disabling a setting unschedules its key on the next reconcile", async () => {
  ...
  const crunch = (await ctx.jobs.listSchedules()).find((s) => s.key === "flow:default");
  assert.equal(crunch?.enabled, true, "the still-enabled crunch schedule is unaffected");
});
...
test("reconcile expands schedules per configured flow", async () => {
  ...
  await ctx.stores.crunchRuns.updateSettings("alpha", { enabled: true, cron: "0 2 * * *" });
  await ctx.stores.scheduledTasks.updateSettings("source-change-sync::beta", { enabled: true, cron: "*/10 * * * *" });

  await reconcileSchedules(ctx);

  assert.deepEqual(await scheduledKeys(ctx), [
    { type: "trigger_scheduled_crunch", key: "flow:alpha", cron: "0 2 * * *" },
