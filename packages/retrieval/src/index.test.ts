...
import { answerQuestion, MockChatProvider, routeQuestionToFlow, type SectionSearchProvider } from "./index.js";
...
describe("answerQuestion", () => {
  ...
  it("forwards repositoryIds to the search provider and persona to the system prompt", async () => {
    let seenRepositoryIds: string[] | undefined = ["sentinel"];
    let seenSystem = "";
    const ranked: RankedSection[] = [
      { section: section("repo:care.md:0", "Cat Care", "Cats need fresh water and a clean litter box."), relevance: 0.5 }
    ];
    const searchProvider: SectionSearchProvider = {
      async search(_question, _limit, repositoryIds) {
        seenRepositoryIds = repositoryIds;
        return ranked;
      }
    };
    ...
    await answerQuestion("How do I care for a cat?", searchProvider, chatProvider, {
      repositoryIds: ["cats"],
      persona: "Answer warmly for new pet owners."
    });

    assert.deepEqual(seenRepositoryIds, ["cats"]);
    assert.match(seenSystem, /Answer warmly for new pet owners\./);
  });
