...
- **Data source note:** ~~jobs live in pg-boss's `"<schema>".job` **and** `"<schema>".archive` (completed/failed rows migrate to `archive` after retention). The throughput rollup MUST `UNION ALL` both tables~~ — **CORRECTION (this guidance was wrong and shipped a bug):** pg-boss **v12** (this repo's version) removed the `archive` table in v10. Every job, live and finished, stays in the partitioned `"<schema>".job` table until retention (`keep_until`) purges it. The rollup MUST read `job` **alone** — the `archive` `UNION ALL` throws `relation "<schema>.archive" does not exist` against a real pg-boss instance. Bucket by `date_trunc(created_on)` and group by state. Read the pg-boss schema name from `ctx.jobs` config (see `apps/api/src/jobs/pg-boss-broker.ts`); guard it with the existing `SCHEMA_IDENTIFIER` regex.
